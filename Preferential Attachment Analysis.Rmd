...
Order matchup by id of Home team lineup and treat time steps as the times that a lineup showed up as a home team. For example, if lineup A showed up as Home team at the first time, we treat this matchup as happened at t_0. The the second time corresponds to t_1.  
...
Add positive outdegree at each time_step for each home lineups. Save results to RData.
```{r message=FALSE, warning=FALSE}
...
Calculate the probability of a home lineup win at current time step if they have positive outdegree = past_deg. 
```{r message=FALSE, warning=FALSE}
...
```{r message=FALSE, warning=FALSE}
home_counts <- count(home_matchup_degree,from) # count number of time steps for each node
...
Order matchup by id of Away team lineup and treat time steps as the times that a lineup showed up as an away team. For example, if lineup A showed up as away team at the first time, we treat this matchup as happened at t_0. The the second time corresponds to t_1.  
...
Add positive indegree at each time_step for each away lineups. Save results to RData.
```{r message=FALSE, warning=FALSE}
...
Calculate the probability of an away lineup win at current time step if they have positive indegree = past_deg. 
```{r message=FALSE, warning=FALSE}
