...
Here is function of calculating ISM metrics
```{r warning=FALSE}
Inout <- function(lineup = new_lineup, matchup, source, target, names){
...
InInOutOut <- function(lineup = new_lineup, matchup, source, target, names){
    act1 <- strsplit(names, split = "_")[[1]][1]
    act2 <- strsplit(names, split = "_")[[1]][2]
    temp_net <- tbl_graph(nodes = lineup, edges = matchup, directed = T,node_key = "label")
    LSP <- distances(temp_net, v=V(temp_net)[id==source], to=V(temp_net)[id==target], mode="all")
...
Now we calculate the ISM metrics for each pair of nodes in 16 network views. (take ~ 2h)
...
for (row in c(1:nrow(pairs))){ # row <- 1
  if (row %% 1000 == 0){
    ...
    # edge_centric[t+1,"node1"] = i
    # edge_centric[t+1,"node2"] = j
    # edge_centric[t+1,"class"] = "LSP"
    ...
    for (k in c(1:16)){
      ...
      if (k %in% c(1:4)){
        ...
        Inout(lineup = new_lineup,matchup = temp_edge,source = j,target = i,names = name_list[k])
    }
      else if (k %in% c(5:8)){
        ...
        InInOutOut(lineup = new_lineup,matchup = temp_edge,source = j,target = i,names = name_list[k])
    }
        ...
        Inout(lineup = new_lineup,matchup = temp_edge,source = i,target = j,names = name_list[k])
    }
...
# write.csv(edge_centric,"edge_centric.csv", row.names = T)
```
...
```{r warning=FALSE}
edge_centric <- read.csv("edge_centric.csv",header = T)
